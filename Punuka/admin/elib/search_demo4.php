...
if (!empty($_POST) ) {
    ...
    $recordsTotal = count(getData("SELECT title, author, edition, status2, position, location, accessNum FROM ".MyTable." WHERE view='enabled'"));
    ...
    if(!empty($_POST['search']['value'])){
        ...
        $sql = sprintf("SELECT title, author, edition, status2, position, location, accessNum FROM %s %s", MyTable , $where);//Search query without limit clause (No pagination)

        $recordsFiltered = count(getData($sql));//Count of search result

        /* SQL Query for search with limit and orderBy clauses*/
        $sql = sprintf("SELECT title, author, edition, status2, position, location, accessNum FROM %s %s ORDER BY %s %s limit %d , %d ", MyTable , $where ,$orderBy, $orderType ,$start,$length  );
        $data = getData($sql);
    }
    /* END SEARCH */
    else {
        $sql = sprintf("SELECT title, author, edition, status2, position, location, accessNum FROM %s ORDER BY %s %s limit %d , %d ", MyTable ,$orderBy,$orderType ,$start , $length);
        $data = getData($sql);
